...
class ExcelOp(object):
    ...
    def get_sheet_par(self, part_name, stamping_press_type):
        parameter_name = self.get_col_cell(1)
        parameter_value = self.get_col_cell(stamping_press_type + 2)
    ...
    def get_assmebly_sheet_par(self, stamping_press_type):
        assmebly_par_list = {}
        part_name = self.get_col_cell(1)
        part_value = self.get_col_cell(stamping_press_type + 2)
        for x in range(len(part_name)):
            assmebly_par_list[part_name[x]] = part_value[x]
        return assmebly_par_list

    def get_assmebly_quantity(self, stamping_press_type):
        part_name = self.get_col_cell(1)
        part_quantity = self.get_col_cell(stamping_press_type + 2)
        return part_name, part_quantity

    def get_standard_parts(self, stamping_press_type):
    ...
    def get_single_data_sheet_par(self, data_name, part_name, stamping_press_type):
        parameter_name = self.get_col_cell(1)
        parameter_value = self.get_col_cell(stamping_press_type + 2)
        position = parameter_name.index(data_name)
        if part_name != '沖床機架零件清單' and part_name != '標準資料':
            mprog.param_change(part_name, parameter_name[position], str(parameter_value[stamping_press_type]))
        else:
            pass
        return parameter_value

    def get_cutout_limit(self, stamping_press_type):
        cutout_dir = self.get_col_cell(1)
        cutout_value = self.get_col_cell(stamping_press_type + 2)
